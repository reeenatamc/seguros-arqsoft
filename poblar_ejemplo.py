...
from app.models import CompaniaAseguradora, CorredorSeguros, DetallePolizaRamo, Poliza, Ramo, TipoPoliza  # noqa: E402
...
compania, _ = CompaniaAseguradora.objects.get_or_create(
    nombre="Tecniseguros", defaults={"ruc": "1790123456001", "activo": True}
)

# Crear corredor
corredor, _ = CorredorSeguros.objects.get_or_create(
    nombre="Broker Agente", defaults={"ruc": "1790654321001", "activo": True}
)
...
for d in detalles_data:
    ...
    print(
        f"Detalle creado: {detalle.ramo.nombre} - Prima: ${detalle.total_prima} - Total Facturado: ${detalle.total_facturado}"
    )
