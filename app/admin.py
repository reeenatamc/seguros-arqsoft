...
class PolizaResource(resources.ModelResource):
    ...
    corredor_seguros = fields.Field(
        column_name="corredor_seguros", attribute="corredor_seguros", widget=ForeignKeyWidget(CorredorSeguros, "nombre")
    )

    tipo_poliza = fields.Field(
        column_name="tipo_poliza", attribute="tipo_poliza", widget=ForeignKeyWidget(TipoPoliza, "nombre")
    )

    fecha_inicio = fields.Field(
        column_name="fecha_inicio", attribute="fecha_inicio", widget=DateWidget(format="%Y-%m-%d")
    )
...
class FacturaResource(resources.ModelResource):
    ...
    poliza = fields.Field(column_name="poliza", attribute="poliza", widget=ForeignKeyWidget(Poliza, "numero_poliza"))

    fecha_emision = fields.Field(
        column_name="fecha_emision", attribute="fecha_emision", widget=DateWidget(format="%Y-%m-%d")
    )

    fecha_vencimiento = fields.Field(
        column_name="fecha_vencimiento", attribute="fecha_vencimiento", widget=DateWidget(format="%Y-%m-%d")
    )
...
class SiniestroResource(resources.ModelResource):
    ...
    tipo_siniestro = fields.Field(
        column_name="tipo_siniestro", attribute="tipo_siniestro", widget=ForeignKeyWidget(TipoSiniestro, "nombre")
    )

    fecha_siniestro = fields.Field(
        column_name="fecha_siniestro", attribute="fecha_siniestro", widget=DateTimeWidget(format="%Y-%m-%d %H:%M:%S")
    )
...
class ConfiguracionSistemaAdmin(ModelAdmin):
    ...
    fieldsets = (
        ("Configuración", {"fields": ("clave", "valor", "tipo")}),
        ("Clasificación", {"fields": ("categoria", "descripcion")}),
    )
...
class CompaniaAseguradoraAdmin(ImportExportModelAdmin):
    ...
    fieldsets = (
        ...
        ("Contacto", {"fields": ("direccion", "telefono", "email", "contacto_nombre", "contacto_telefono")}),
...
class ResponsableCustodioAdmin(ImportExportModelAdmin):
    ...
    fieldsets = (
        ("Información Personal", {"fields": ("nombre", "cargo", "departamento")}),
...
class PolizaAdmin(HistoryModelAdmin):
    ...
    list_filter = ["estado", "compania_aseguradora", "tipo_poliza", "fecha_inicio", "fecha_fin"]

    search_fields = ["numero_poliza", "compania_aseguradora__nombre", "corredor_seguros__nombre", "coberturas"]
    ...
    fieldsets = (
        ...
        ("Coberturas y Montos", {"fields": ("suma_asegurada", "coberturas")}),
        (
            "Vigencia",
        ...
        ("Auditoría", {"fields": ("creado_por", "fecha_creacion", "fecha_modificacion"), "classes": ("collapse",)}),
    )
    ...
    def estado_badge(self, obj):
        ...
        colors = {"vigente": "green", "vencida": "red", "cancelada": "gray", "por_vencer": "orange"}
...
class FacturaAdmin(HistoryModelAdmin):
    ...
    list_filter = ["estado", "fecha_emision", "fecha_vencimiento", "poliza__compania_aseguradora"]

    search_fields = ["numero_factura", "poliza__numero_poliza", "poliza__compania_aseguradora__nombre"]
    ...
    fieldsets = (
        ("Información Básica", {"fields": ("poliza", "numero_factura", "fecha_emision", "fecha_vencimiento")}),
        (
            "Montos",
            {
                "fields": (
        ...
        ("Estado y Descuentos", {"fields": ("estado", "saldo_pendiente_display", "puede_aplicar_descuento_display")}),
        ("Auditoría", {"fields": ("creado_por", "fecha_creacion", "fecha_modificacion"), "classes": ("collapse",)}),
    )
    ...
    def estado_badge(self, obj):
        ...
        colors = {"pendiente": "orange", "pagada": "green", "parcial": "blue", "vencida": "red"}
    ...
    def saldo_pendiente_display(self, obj):
        ...
        return format_html('<span style="color: {}; font-weight: bold;">{}</span>', color, saldo_formatted)
...
class PagoAdmin(HistoryModelAdmin):
    ...
    list_display = ["referencia", "factura", "fecha_pago", "monto_formatted", "forma_pago", "estado_badge"]

    list_filter = ["estado", "forma_pago", "fecha_pago"]

    search_fields = ["referencia", "factura__numero_factura", "factura__poliza__numero_poliza"]
    ...
    fieldsets = (
        ("Información del Pago", {"fields": ("factura", "fecha_pago", "monto", "forma_pago", "referencia")}),
        ("Estado", {"fields": ("estado", "observaciones")}),
        ("Auditoría", {"fields": ("registrado_por", "fecha_creacion"), "classes": ("collapse",)}),
    )
    ...
    def estado_badge(self, obj):
        ...
        colors = {"pendiente": "orange", "aprobado": "green", "rechazado": "red"}
...
class SiniestroAdmin(HistoryModelAdmin):
    ...
    fieldsets = (
        ("Información Básica", {"fields": ("numero_siniestro", "poliza", "tipo_siniestro", "fecha_siniestro")}),
        (
            "Bien Asegurado",
            {"fields": ("bien_nombre", "bien_modelo", "bien_serie", "bien_marca", "bien_codigo_activo")},
        ),
        ("Detalles del Siniestro", {"fields": ("responsable_custodio", "ubicacion", "causa", "descripcion_detallada")}),
        ("Montos", {"fields": ("monto_estimado", "monto_indemnizado")}),
        (
            "Estado y Gestión",
            {
                "fields": (
        ...
        ("Observaciones", {"fields": ("observaciones",), "classes": ("collapse",)}),
        ("Auditoría", {"fields": ("creado_por", "fecha_registro", "fecha_modificacion"), "classes": ("collapse",)}),
    )
...
class DocumentoAdmin(HistoryModelAdmin):
    ...
    fieldsets = (
        ...
        ("Relaciones", {"fields": ("poliza", "siniestro", "factura")}),
        ("Auditoría", {"fields": ("subido_por", "fecha_subida"), "classes": ("collapse",)}),
    )
...
class AlertaAdmin(ModelAdmin):
    ...
    fieldsets = (
        ("Información de la Alerta", {"fields": ("tipo_alerta", "titulo", "mensaje", "estado")}),
        ("Relaciones", {"fields": ("poliza", "factura", "siniestro")}),
        ("Destinatarios", {"fields": ("destinatarios",)}),
        ("Auditoría", {"fields": ("fecha_creacion", "fecha_envio", "fecha_lectura"), "classes": ("collapse",)}),
    )

    @display(description="Estado", ordering="estado")
    def estado_badge(self, obj):

        colors = {"pendiente": "orange", "enviada": "blue", "leida": "purple", "atendida": "green"}
...
class BienAseguradoAdmin(HistoryModelAdmin):
    ...
    fieldsets = (
        ("Identificación", {"fields": ("codigo_bien", "nombre", "descripcion", "categoria")}),
        ("Clasificación de Seguro", {"fields": ("poliza", "subgrupo_ramo", "clasificacion_completa")}),
        ("Detalles Técnicos", {"fields": ("marca", "modelo", "serie", "codigo_activo", "anio_fabricacion")}),
        ("Imagen e Identificación", {"fields": ("imagen", "codigo_qr"), "classes": ("collapse",)}),
        ("Ubicación", {"fields": ("ubicacion", "edificio", "piso", "departamento")}),
        ("Responsable", {"fields": ("responsable_custodio",)}),
        ("Valores Financieros", {"fields": ("valor_compra", "valor_actual", "valor_asegurado", "valor_comercial")}),
        ("Fechas", {"fields": ("fecha_adquisicion", "fecha_garantia")}),
        ("Estado", {"fields": ("estado", "condicion", "activo")}),
        ("Agrupación", {"fields": ("grupo_bienes",), "classes": ("collapse",)}),
        ("Observaciones", {"fields": ("observaciones",), "classes": ("collapse",)}),
        ("Auditoría", {"fields": ("creado_por", "fecha_creacion", "fecha_modificacion"), "classes": ("collapse",)}),
    )
    ...
    def estado_badge(self, obj):
        ...
        return format_html('<span class="badge {}">{}</span>', color, obj.get_estado_display())
    ...
    def condicion_badge(self, obj):
        ...
        return format_html('<span class="badge {}">{}</span>', color, obj.get_condicion_display())
...
class QuoteAdmin(HistoryModelAdmin):
    ...
    list_filter = ["status", "priority", "policy_type", "request_date"]

    search_fields = ["quote_number", "title", "coverage_details"]
    ...
    fieldsets = (
        ("Identificación", {"fields": ("quote_number", "title", "policy_type", "priority")}),
        ("Valores y Coberturas", {"fields": ("sum_insured", "coverage_details")}),
        (
            "Fechas",
            {
                "fields": (
        ...
        ("Estado", {"fields": ("status", "resulting_policy", "rejection_reason")}),
        ("Notas", {"fields": ("notes",), "classes": ("collapse",)}),
        ("Auditoría", {"fields": ("requested_by", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def days_until_expiry_display(self, obj):
        ...
        return format_html('<span style="color: {}; font-weight: bold;">{} días</span>', color, days)
...
class QuoteOptionAdmin(ModelAdmin):
    ...
    list_filter = ["is_recommended", "insurer", "broker", "received_date"]

    search_fields = ["quote__quote_number", "insurer__nombre", "broker__nombre"]

    readonly_fields = ["created_at", "updated_at", "premium_per_thousand_display"]

    fieldsets = (
        ("Cotización", {"fields": ("quote",)}),
        ("Proveedor", {"fields": ("insurer", "broker")}),
        ("Valores", {"fields": ("premium_amount", "deductible", "premium_per_thousand_display")}),
        ("Cobertura", {"fields": ("coverage_offered", "exclusions", "conditions")}),
        ("Documento", {"fields": ("proposal_document",)}),
        ("Evaluación", {"fields": ("is_recommended", "rating", "evaluation_notes")}),
        ("Fechas", {"fields": ("received_date", "valid_until")}),
...
class PolicyRenewalAdmin(HistoryModelAdmin):
    ...
    list_filter = ["status", "decision", "due_date", "original_policy__compania_aseguradora"]

    search_fields = ["renewal_number", "original_policy__numero_poliza", "new_policy__numero_poliza"]
    ...
    fieldsets = (
        ("Póliza a Renovar", {"fields": ("original_policy", "renewal_number")}),
        (
            "Fechas del Proceso",
            {
                "fields": (
        ...
        (
            ...
            {"fields": ("original_premium", "proposed_premium", "final_premium", "premium_change_percentage_display")},
        ),
        ("Cambios en Cobertura", {"fields": ("coverage_changes",)}),
        ("Estado y Decisión", {"fields": ("status", "decision", "decision_reason")}),
        ("Cotización Asociada", {"fields": ("quote",), "classes": ("collapse",)}),
        ("Resultado", {"fields": ("new_policy",)}),
        ("Notificaciones", {"fields": ("reminder_sent", "reminder_date"), "classes": ("collapse",)}),
        ("Notas", {"fields": ("notes",), "classes": ("collapse",)}),
        ("Auditoría", {"fields": ("created_by", "approved_by", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def decision_badge(self, obj):
        ...
        colors = {"renew_same": "#10b981", "renew_different": "#3b82f6", "not_renew": "#ef4444", "pending": "#f59e0b"}

        color = colors.get(obj.decision, "#6b7280")

        return format_html('<span style="color: {}; font-weight: bold;">{}</span>', color, obj.get_decision_display())
    ...
    def premium_comparison(self, obj):
        ...
        return format_html('<span style="color: {}; font-weight: bold;">{} {}</span>', color, arrow, pct_formatted)
...
class PaymentApprovalAdmin(ModelAdmin):
    ...
    list_filter = ["status", "approval_level", "required_level", "requested_at"]

    search_fields = ["payment__factura__numero_factura", "payment__referencia", "approver__username"]

    readonly_fields = ["requested_at", "decided_at"]

    fieldsets = (
        ("Pago", {"fields": ("payment",)}),
        ("Niveles de Aprobación", {"fields": ("approval_level", "required_level")}),
        ("Estado", {"fields": ("status", "digital_signature")}),
        ("Aprobador", {"fields": ("approver", "decided_at")}),
        ("Notas", {"fields": ("request_notes", "decision_notes")}),
        ("Auditoría", {"fields": ("requested_at",), "classes": ("collapse",)}),
    )

    @display(description="Nivel Aprobación", ordering="approval_level")
    def approval_level_badge(self, obj):

        colors = {"level_1": "#6b7280", "level_2": "#3b82f6", "level_3": "#f59e0b", "level_4": "#ef4444"}
        ...
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 10px; border-radius: 3px;">{}</span>',
            color,
            obj.get_approval_level_display(),
        )
    ...
    def required_level_badge(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>', color, obj.get_required_level_display()
        )

    @display(description="Estado", ordering="status")
    def status_badge(self, obj):

        colors = {"pending": "#f59e0b", "approved": "#10b981", "rejected": "#ef4444", "cancelled": "#6b7280"}
...
class CalendarEventAdmin(ModelAdmin):
    ...
    list_filter = ["event_type", "priority", "is_completed", "is_auto_generated", "start_date"]

    search_fields = ["title", "description", "policy__numero_poliza", "invoice__numero_factura"]

    readonly_fields = ["created_at", "updated_at", "created_by", "is_auto_generated", "color_preview"]

    date_hierarchy = "start_date"

    fieldsets = (
        ("Información del Evento", {"fields": ("title", "description", "event_type", "priority", "color_preview")}),
        ("Fechas y Horarios", {"fields": ("start_date", "end_date", "all_day", "start_time", "end_time")}),
        ("Relaciones", {"fields": ("policy", "invoice", "renewal", "claim", "quote"), "classes": ("collapse",)}),
        ("Notificaciones", {"fields": ("reminder_days", "reminder_sent")}),
        ("Estado", {"fields": ("is_completed", "is_auto_generated")}),
    ...
    def priority_badge(self, obj):
        ...
        colors = {"low": "#6b7280", "medium": "#3b82f6", "high": "#f59e0b", "critical": "#ef4444"}
