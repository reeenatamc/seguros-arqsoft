...
class FileValidator:
    ...
    def __init__(self, max_size=DEFAULT_MAX_FILE_SIZE, allowed_extensions=None, allowed_mime_types=None):
    ...
    def _validate_extension(self, file):
        ...
        if ext in DANGEROUS_EXTENSIONS:
            ...
            raise ValidationError(f'El tipo de archivo "{ext}" no está permitido por razones de seguridad.')
...
class ImageValidator(FileValidator):
    ...
    ALLOWED_IMAGE_MIME_TYPES = ["image/jpeg", "image/png", "image/gif", "image/webp", "image/tiff"]
...
class PDFValidator(FileValidator):
    ...
    def __init__(self, max_size=20 * 1024 * 1024):  # 20MB por defecto para PDFs
        ...
        super().__init__(max_size=max_size, allowed_extensions=[".pdf"], allowed_mime_types=["application/pdf"])
...
def validate_file_extension(value):
    ...
    if ext in DANGEROUS_EXTENSIONS:
        ...
        raise ValidationError(f'El tipo de archivo "{ext}" no está permitido.')

    if ext not in ALLOWED_EXTENSIONS:

        raise ValidationError(
            f'Extensión no permitida: "{ext}". ' f"Use: PDF, imágenes (JPG, PNG), o documentos Office."
        )
