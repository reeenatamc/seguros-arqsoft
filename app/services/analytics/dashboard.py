...
class DashboardAnalyticsService:
    ...
    MONTH_NAMES_SHORT_ES = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"]

    @classmethod
    def get_date_range_for_period(
        cls, period_type: str, reference_date: Optional[date] = None, offset: int = 0
    ) -> Tuple[date, date]:
        """
    ...
    def get_comparative_stats(
        cls, period_type: str = PERIOD_MONTH, reference_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """
        ...
        from app.models import Alerta, Factura, Poliza, Siniestro

        # Calcular rangos de fechas

        current_start, current_end = cls.get_date_range_for_period(period_type, reference_date, offset=0)

        previous_start, previous_end = cls.get_date_range_for_period(period_type, reference_date, offset=1)
        ...
        return {
            ...
            "policies": {
                "new_count": calculate_variation(
                    Decimal(current_policies["new_count"]), Decimal(previous_policies["new_count"])
                ),
                "total_insured": calculate_variation(
                    current_policies["total_insured"], previous_policies["total_insured"]
                ),
                "expired_count": calculate_variation(
                    Decimal(current_policies["expired_count"]), Decimal(previous_policies["expired_count"])
                ),
            },
            "invoices": {
                "count": calculate_variation(Decimal(current_invoices["count"]), Decimal(previous_invoices["count"])),
                "total_amount": calculate_variation(
                    current_invoices["total_amount"], previous_invoices["total_amount"]
                ),
                "paid_amount": calculate_variation(current_invoices["paid_amount"], previous_invoices["paid_amount"]),
                "collection_rate": {
                    "current": current_invoices["collection_rate"],
                    "previous": previous_invoices["collection_rate"],
                    "trend": (
                        "up" if current_invoices["collection_rate"] > previous_invoices["collection_rate"] else "down"
                    ),
                },
            },
            "claims": {
                "count": calculate_variation(Decimal(current_claims["count"]), Decimal(previous_claims["count"])),
                "estimated_amount": calculate_variation(
                    current_claims["estimated_amount"], previous_claims["estimated_amount"]
                ),
                "indemnified_amount": calculate_variation(
                    current_claims["indemnified_amount"], previous_claims["indemnified_amount"]
                ),
            },
        }
    ...
    def _get_policy_stats(cls, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        new_policies = Poliza.objects.filter(fecha_creacion__date__gte=start_date, fecha_creacion__date__lte=end_date)

        # Pólizas vencidas en el período

        expired_policies = Poliza.objects.filter(fecha_fin__gte=start_date, fecha_fin__lte=end_date, estado="vencida")

        new_count = new_policies.count()

        total_insured = new_policies.aggregate(total=Coalesce(Sum("suma_asegurada"), Value(Decimal("0"))))["total"]
    ...
    def _get_invoice_stats(cls, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        invoices = Factura.objects.filter(fecha_emision__gte=start_date, fecha_emision__lte=end_date)

        count = invoices.count()

        total_amount = invoices.aggregate(total=Coalesce(Sum("monto_total"), Value(Decimal("0"))))["total"]
    ...
    def _get_claim_stats(cls, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        claims = Siniestro.objects.filter(fecha_siniestro__date__gte=start_date, fecha_siniestro__date__lte=end_date)

        count = claims.count()

        estimated = claims.aggregate(total=Coalesce(Sum("monto_estimado"), Value(Decimal("0"))))["total"]

        indemnified = claims.aggregate(total=Coalesce(Sum("monto_indemnizado"), Value(Decimal("0"))))["total"]
    ...
    @classmethod
    def get_trend_data(cls, period_type: str = PERIOD_MONTH, periods_count: int = 12) -> Dict[str, Any]:
        """
        ...
        for offset in range(periods_count - 1, -1, -1):
            ...
            policies_count = Poliza.objects.filter(
                fecha_creacion__date__gte=start_date, fecha_creacion__date__lte=end_date
            ).count()
            ...
            invoice_total = Factura.objects.filter(
                fecha_emision__gte=start_date, fecha_emision__lte=end_date
            ).aggregate(total=Coalesce(Sum("monto_total"), Value(Decimal("0"))))["total"]
            ...
            claims_queryset = Siniestro.objects.filter(
                fecha_siniestro__date__gte=start_date, fecha_siniestro__date__lte=end_date
            )

            claims_count = claims_queryset.count()

            claims_amount = claims_queryset.aggregate(total=Coalesce(Sum("monto_estimado"), Value(Decimal("0"))))[
                "total"
            ]
        ...
        return {
            "labels": labels,
            "datasets": {
                "policies": {"label": "Pólizas Nuevas", "data": policies_data, "color": "#3B82F6"},  # blue-500
                "invoices_amount": {
                    "label": "Facturación ($)",
                    "data": invoices_amount_data,
                    "color": "#10B981",  # emerald-500
                },
                "claims_count": {"label": "Siniestros", "data": claims_data, "color": "#EF4444"},  # red-500
                "claims_amount": {
    ...
    def get_year_over_year_comparison(cls) -> Dict[str, Any]:
        ...
        for month in range(1, today.month + 1):
            ...
            current_invoice_total = Factura.objects.filter(
                fecha_emision__year=current_year, fecha_emision__month=month
            ).aggregate(total=Coalesce(Sum("monto_total"), Value(Decimal("0"))))["total"]

            current_year_invoices.append(float(current_invoice_total))

            current_claim_count = Siniestro.objects.filter(
                fecha_siniestro__year=current_year, fecha_siniestro__month=month
            ).count()
            ...
            prev_invoice_total = Factura.objects.filter(
                fecha_emision__year=previous_year, fecha_emision__month=month
            ).aggregate(total=Coalesce(Sum("monto_total"), Value(Decimal("0"))))["total"]

            previous_year_invoices.append(float(prev_invoice_total))

            prev_claim_count = Siniestro.objects.filter(
                fecha_siniestro__year=previous_year, fecha_siniestro__month=month
            ).count()
    ...
    def get_quick_actions_data(cls) -> Dict[str, Any]:
        ...
        return {
            "policies_expiring_soon": Poliza.objects.filter(
                fecha_fin__gte=today, fecha_fin__lte=in_7_days, estado__in=["vigente", "por_vencer"]
            ).count(),
            "policies_expiring_month": Poliza.objects.filter(
                fecha_fin__gte=today, fecha_fin__lte=in_30_days, estado__in=["vigente", "por_vencer"]
            ).count(),
            "overdue_invoices": Factura.objects.filter(estado="vencida").count(),
            "pending_claims_docs": Siniestro.objects.filter(estado="documentacion_pendiente").count(),
            "claims_awaiting_response": Siniestro.objects.filter(
                estado="enviado_aseguradora", fecha_envio_aseguradora__lte=today - timedelta(days=8)
            ).count(),
            "unread_alerts": Alerta.objects.filter(estado__in=["pendiente", "enviada"]).count(),
        }
    ...
    def get_top_performers(cls, limit: int = 5) -> Dict[str, Any]:
        ...
        from app.models import CompaniaAseguradora, Poliza, Siniestro
        ...
        top_insurers_by_policies = list(
            ...
            .annotate(count=Count("id"), total_insured=Sum("suma_asegurada"))
        ...
        top_insurers_by_claims = list(
            ...
            .annotate(count=Count("id"), total_amount=Sum("monto_estimado"))
            .order_by("-count")[:limit]
        )
    ...
    @classmethod
    def get_dashboard_summary(cls, period_type: str = PERIOD_MONTH) -> Dict[str, Any]:
        """
