...
from django.db.models.functions import ExtractMonth, TruncMonth, TruncWeek
...
class EstadisticasService:
    ...
    def get_dashboard_stats():
        ...
        hoy = timezone.now().date()  # noqa: F841
        ...
        return {
            ...
            "facturas": {
                ...
                "monto_pendiente": Factura.objects.filter(estado__in=["pendiente", "parcial"]).aggregate(
    ...
    def get_polizas_por_estado():
        ...
        estados = (
            ...
            .annotate(cantidad=Count("id"), suma_total=Sum("suma_asegurada"))
            .order_by("estado")
        )
    ...
    def get_polizas_por_compania():
        ...
        return list(
            Poliza.objects.values("compania_aseguradora__nombre")
            .annotate(cantidad=Count("id"), suma_total=Sum("suma_asegurada"))
    ...
    def get_siniestros_por_tipo():
        ...
        return list(
            ...
            .annotate(
                cantidad=Count("id"), monto_total=Sum("monto_estimado"), monto_indemnizado=Sum("monto_indemnizado")
            )
            .order_by("-cantidad")
        )
    ...
    def get_siniestros_por_mes(meses=12):
        ...
        return list(
            Siniestro.objects.filter(fecha_siniestro__gte=fecha_inicio)
            .annotate(mes=TruncMonth("fecha_siniestro"))
            .values("mes")
            .annotate(cantidad=Count("id"), monto_total=Sum("monto_estimado"))
            .order_by("mes")
        )
    ...
    def get_facturas_por_mes(meses=12):
        ...
        return list(
            Factura.objects.filter(fecha_emision__gte=fecha_inicio)
    ...
    def get_polizas_proximas_vencer(dias=30, limit=10):
        ...
        return (
            Poliza.objects.filter(fecha_fin__gte=hoy, fecha_fin__lte=fecha_limite, estado__in=["vigente", "por_vencer"])
    ...
    def get_siniestros_recientes(limit=10):
        ...
        return (
            ...
            .select_related("poliza", "tipo_siniestro", "poliza__compania_aseguradora")
    ...
    def get_kpis():
        ...
        hace_30_dias = hoy - timedelta(days=30)  # noqa: F841

        hace_365_dias = hoy - timedelta(days=365)

        polizas_vigentes = Poliza.objects.filter(estado="vigente")

        suma_total_asegurada = polizas_vigentes.aggregate(total=Sum("suma_asegurada"))["total"] or Decimal("0")
        ...
        tiempo_promedio_resolucion = (
            siniestros_anio.filter(fecha_liquidacion__isnull=False)
            .annotate(dias_resolucion=F("fecha_liquidacion") - F("fecha_registro"))
