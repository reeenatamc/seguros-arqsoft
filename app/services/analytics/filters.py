...
class DashboardFiltersService:
    ...
    def get_available_filters(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "insurers": list(CompaniaAseguradora.objects.filter(activo=True).values("id", "nombre").order_by("nombre")),
            "brokers": list(CorredorSeguros.objects.filter(activo=True).values("id", "nombre").order_by("nombre")),
            "policy_types": list(TipoPoliza.objects.filter(activo=True).values("id", "nombre").order_by("nombre")),
            "claim_types": list(TipoSiniestro.objects.filter(activo=True).values("id", "nombre").order_by("nombre")),
            "policy_states": [{"value": choice[0], "label": choice[1]} for choice in Poliza.ESTADO_CHOICES],
            "invoice_states": [{"value": choice[0], "label": choice[1]} for choice in Factura.ESTADO_CHOICES],
            "claim_states": [{"value": choice[0], "label": choice[1]} for choice in Siniestro.ESTADO_CHOICES],
        }
    ...
    def get_filtered_stats(cls, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        invoice_stats = invoices_qs.aggregate(
            ...
            pending_amount=Coalesce(
                Sum("monto_total", filter=Q(estado__in=["pendiente", "parcial", "vencida"])), Value(Decimal("0"))
            ),
        )
        ...
        if invoice_stats["total_amount"] > 0:
            ...
            collection_rate = round((invoice_stats["paid_amount"] / invoice_stats["total_amount"]) * 100, 1)

        loss_ratio = 0

        if invoice_stats["paid_amount"] > 0:

            loss_ratio = round((claim_stats["total_indemnified"] / invoice_stats["paid_amount"]) * 100, 1)
    ...
    def get_chart_data(cls, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        invoicing_by_period = list(
            invoices_qs.annotate(period=trunc_func("fecha_emision"))
            .values("period")
            .annotate(
        ...
        claims_by_period = list(
            claims_qs.annotate(period=trunc_func("fecha_siniestro"))
            .values("period")
            .annotate(count=Count("id"), amount=Coalesce(Sum("monto_estimado"), Value(Decimal("0"))))
            .order_by("period")
        )
        ...
        invoicing_labels = [
            item["period"].strftime(date_format) if item["period"] else "" for item in invoicing_by_period
        ]

        claims_labels = [item["period"].strftime(date_format) if item["period"] else "" for item in claims_by_period]
        ...
        by_insurer = list(
            policies_qs.values("compania_aseguradora__nombre")
            .annotate(count=Count("id"), total=Coalesce(Sum("suma_asegurada"), Value(Decimal("0"))))
            .order_by("-count")[:8]
        )

        # Distribución por tipo de póliza

        by_policy_type = list(policies_qs.values("tipo_poliza__nombre").annotate(count=Count("id")).order_by("-count"))

        # Distribución por estado de póliza

        by_policy_state = list(policies_qs.values("estado").annotate(count=Count("id")).order_by("estado"))

        # Distribución por tipo de siniestro (sin límite para mostrar todos)

        by_claim_type = list(
            claims_qs.values("tipo_siniestro__nombre")
            .annotate(count=Count("id"), amount=Coalesce(Sum("monto_estimado"), Value(Decimal("0"))))
            .order_by("-count")
        )
    ...
    def get_lists_data(cls, filters: Dict[str, Any], limit: int = 5) -> Dict[str, Any]:
        ...
        expiring_policies = list(
            Poliza.objects.filter(
                fecha_fin__gte=today, fecha_fin__lte=today + timedelta(days=30), estado__in=["vigente", "por_vencer"]
            )
    ...
    def _build_invoice_queryset(cls, filters: Dict[str, Any]):
        ...
        if filters.get("search_query"):
            ...
            qs = qs.filter(Q(numero_factura__icontains=q) | Q(poliza__numero_poliza__icontains=q))
    ...
    def _build_claim_queryset(cls, filters: Dict[str, Any]):
        ...
        if date_from and date_to:
            ...
            qs = qs.filter(fecha_siniestro__date__gte=date_from, fecha_siniestro__date__lte=date_to)
        ...
        if filters.get("search_query"):
            ...
            qs = qs.filter(
                Q(numero_siniestro__icontains=q) | Q(bien_nombre__icontains=q) | Q(poliza__numero_poliza__icontains=q)
            )
