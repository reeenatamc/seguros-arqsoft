...
class AdvancedAnalyticsService:
    ...
    def get_claims_vs_premiums_trend(cls, months=12):
        ...
        premiums_by_month = (
            Pago.objects.filter(estado="aprobado", fecha_pago__gte=start_date)
        ...
        claims_by_month = (
            Siniestro.objects.filter(estado__in=["liquidado", "cerrado"], fecha_liquidacion__gte=start_date)
            .annotate(month=TruncMonth("fecha_liquidacion"))
            .values("month")
            .annotate(total=Sum("monto_indemnizado"))
            .order_by("month")
        )
    ...
    def get_claims_by_location(cls):
        ...
        locations_data = (
            Siniestro.objects.exclude(ubicacion__isnull=True)
            .exclude(ubicacion="")
            .values("ubicacion")
            .annotate(
                count=Count("id"),
    ...
    def _predict_single_policy(cls, policy_id):
        ...
        monto_siniestros = float(
            siniestros.aggregate(total=Coalesce(Sum("monto_indemnizado"), Value(Decimal("0"))))["total"]
        )
    ...
    def _predict_all_policies(cls):
        ...
        polizas_por_renovar = Poliza.objects.filter(
            estado__in=["vigente", "por_vencer"], fecha_fin__gte=today, fecha_fin__lte=today + timedelta(days=90)
        ).order_by("fecha_fin")
    ...
    def get_insurer_performance(cls):
        ...
        for insurer in insurers:
            ...
            _tiempos = siniestros.filter(  # noqa: F841
                fecha_envio_aseguradora__isnull=False, fecha_respuesta_aseguradora__isnull=False
            ).annotate(dias=F("fecha_respuesta_aseguradora") - F("fecha_envio_aseguradora"))

            # Monto total indemnizado

            monto_indemnizado = float(
                siniestros.aggregate(total=Coalesce(Sum("monto_indemnizado"), Value(Decimal("0"))))["total"]
            )
    ...
    def get_dashboard_summary(cls):
        ...
        total_primas_year = Pago.objects.filter(estado="aprobado", fecha_pago__gte=start_year).aggregate(
            total=Coalesce(Sum("monto"), Value(Decimal("0")))
        )["total"]

        total_indemnizaciones_year = Siniestro.objects.filter(
            estado__in=["liquidado", "cerrado"], fecha_liquidacion__gte=start_year
        ).aggregate(total=Coalesce(Sum("monto_indemnizado"), Value(Decimal("0"))))["total"]
        ...
        siniestros_abiertos = Siniestro.objects.exclude(estado__in=["cerrado", "rechazado"]).count()
