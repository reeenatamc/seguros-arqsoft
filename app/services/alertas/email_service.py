...
from app.models import ConfiguracionSistema, Factura, NotificacionEmail, Poliza, Siniestro
...
class NotificacionesService:
            ...
            "bloques": [
                {
                    ...
                    "filas": [
                        {"label": "Número", "valor": siniestro.numero_siniestro},
                        {
                            "label": "Tipo",
                            "valor": (
                                siniestro.tipo_siniestro.get_nombre_display() if siniestro.tipo_siniestro else "N/A"
                            ),
                        },
                        {
            ...
            "bloques": [
                ...
                },
                {
                    ...
                    "filas": [
                        ...
                        {
                            "label": "Fecha de Pago",
                            "valor": (siniestro.fecha_pago.strftime("%d/%m/%Y") if siniestro.fecha_pago else "N/A"),
                        },
                    ],
                },
            ],
    ...
    def _crear_alerta_respuesta(cls, siniestro):
        ...
        alerta_reciente = NotificacionEmail.objects.filter(
            tipo="alerta_respuesta", siniestro=siniestro, fecha_creacion__gte=timezone.now() - timedelta(days=2)
        ).exists()
    ...
    def _crear_alerta_deposito(cls, siniestro):
        ...
        alerta_reciente = NotificacionEmail.objects.filter(
            tipo="alerta_deposito", siniestro=siniestro, fecha_creacion__gte=timezone.now() - timedelta(days=1)
        ).exists()
        ...
        horas_transcurridas = int((timezone.now() - siniestro.fecha_firma_indemnizacion).total_seconds() / 3600)
