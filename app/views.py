...
def _generar_preview_aseguradora(siniestro):
    ...
    adjuntos_lista = [
        {'nombre': adj.nombre, 'tipo': adj.get_tipo_adjunto_display()}
        for adj in adjuntos
    ]
...
def siniestro_firmar_recibo(request, pk):
    ...
    return render(request, 'app/siniestros/firmar_recibo.html', {
        'siniestro': siniestro,
    })
...
def siniestro_disputar(request, pk):
    ...
    return render(request, 'app/siniestros/disputar.html', {
        'siniestro': siniestro,
    })
...
def siniestro_resolver_disputa(request, pk):
    ...
    return render(request, 'app/siniestros/resolver_disputa.html', {
        'siniestro': siniestro,
    })
        ...
        if poliza and poliza.compania_aseguradora and poliza.compania_aseguradora.email:
            try:
                ...
                context = {
                    ...
                    'bien_nombre': bien_nombre,
                    'fecha_firma': siniestro.fecha_firma_indemnizacion.strftime('%d/%m/%Y') if siniestro.fecha_firma_indemnizacion else timezone.now().strftime('%d/%m/%Y'),
                    'fecha_limite': siniestro.fecha_limite_liquidacion.strftime('%d/%m/%Y %H:%M') if siniestro.fecha_limite_liquidacion else 'N/A',
                    'firmante_nombre': firmante_nombre,
                    'firmante_cargo': firmante_cargo,
                    'firmante_departamento': firmante_departamento,
                    'firmante_telefono': firmante_telefono,
...
"""
                ...
                email.send()
                messages.success(request, f'Recibo enviado a {poliza.compania_aseguradora.email}. Plazo de 72h hábiles iniciado.')
            except Exception as e:
    ...
    return render(request, 'app/siniestros/enviar_liquidacion.html', {
        'siniestro': siniestro,
    })
    ...
    if request.method == 'POST':
        try:
            ...
            if hay_diferencia and not confirmar_diferencia:
                ...
                return render(request, 'app/siniestros/registrar_liquidacion.html', {
                    'siniestro': siniestro,
                    'monto': monto,
...
def siniestro_cerrar(request, pk):
    ...
    return render(request, 'app/siniestros/cerrar.html', {
        'siniestro': siniestro,
    })
...
def siniestro_rechazar(request, pk):
    ...
    return render(request, 'app/siniestros/rechazar.html', {
        'siniestro': siniestro,
    })
...
def _enviar_notificaciones_cierre(siniestro):
    ...
    if siniestro.responsable_custodio and siniestro.responsable_custodio.email:
        ...
        destinatarios_info.append({
            'email': siniestro.responsable_custodio.email,
            'nombre': nombre
        })
