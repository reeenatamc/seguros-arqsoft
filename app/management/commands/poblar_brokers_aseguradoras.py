    ...
    def handle(self, *args, **options):
        ...
        for poliza in Poliza.objects.select_related("compania_aseguradora", "corredor_seguros"):
            ...
            if not compania.brokers.filter(pk=broker.pk).exists():
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Vinculado broker '{broker.nombre}' con aseguradora '{compania.nombre}'.")
                )

        self.stdout.write(self.style.SUCCESS(f"Proceso completado. Relaciones nuevas creadas: {total_relaciones}"))
