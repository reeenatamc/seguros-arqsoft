...
class Command(BaseCommand):
    ...
    def crear_tipos_poliza(self):
        ...
        for data in tipos_data:
            tipo, _ = TipoPoliza.objects.get_or_create(
                nombre=data["nombre"], defaults={"descripcion": data["descripcion"], "activo": True}
            )
            tipos.append(tipo)
        return tipos
    ...
    def crear_tipos_siniestro(self):
        ...
        for data in tipos_data:
            tipo, _ = TipoSiniestro.objects.get_or_create(
                nombre=data["nombre"], defaults={"descripcion": data.get("descripcion", ""), "activo": True}
            )
            tipos.append(tipo)
        return tipos
    ...
    def crear_pagos(self, facturas, usuario):
        ...
        for factura in facturas:
            ...
            tipo_pago = random.choices(["completo", "parcial", "multiple"], weights=[0.6, 0.25, 0.15])[0]
    ...
    def crear_responsables(self):
        ...
        cargos = ["Jefe de Departamento", "Coordinador", "Supervisor", "Analista", "Técnico", "Asistente"]
        nombres = [
            "Juan Pérez",
            "María González",
            "Carlos Rodríguez",
    ...
    def crear_siniestros(self, polizas, tipos_siniestro, responsables, usuario):
        ...
        ubicaciones = [
            ...
            "Edificio B, Piso 1",
            "Edificio B, Piso 2",
            "Edificio C, Oficina 101",
            "Edificio C, Oficina 205",
            "Bodega Central",
            "Parqueadero Norte",
            "Parqueadero Sur",
            "Laboratorio 1",
    ...
    def _crear_siniestro(
        ...
        if estado in ["aprobado", "cerrado"]:
            siniestro.fecha_respuesta_aseguradora = siniestro.fecha_envio_aseguradora + timedelta(
    ...
    def crear_alertas(self, polizas, facturas, siniestros):
        ...
        for siniestro in siniestros:
            if siniestro.estado == "documentacion_pendiente":
                ...
                dias_esperando = (
                    (hoy - siniestro.fecha_envio_aseguradora).days if siniestro.fecha_envio_aseguradora else 0
                )
                if dias_esperando > 7:
                    alerta, created = Alerta.objects.get_or_create(
    ...
    def create_insured_assets(self, polizas, responsables, usuario):
        ...
        sample_assets = [
            ...
            },
            {
                ...
                "brand": "Cisco",
                "model": "ISR 4331",
                "category": "Equipos de Comunicación",
                "value": 3500,
            },
            {
                "name": "Switch Cisco",
        ...
        if not subgrupos:
            self.stdout.write(
                self.style.WARNING("No hay SubgrupoRamo disponibles. Ejecuta poblar_catalogo_ramos primero.")
            )
            return assets
        ...
        for i in range(num_assets):
            ...
            estado_en = random.choices(["active", "inactive", "disposed"], weights=[0.85, 0.10, 0.05])[0]
            condicion_en = random.choices(["excellent", "good", "fair", "poor"], weights=[0.2, 0.5, 0.25, 0.05])[0]
    ...
    def create_quotes(self, companias, corredores, tipos_poliza, usuario):
        ...
        for i in range(num_quotes):
            ...
            quote = Quote.objects.create(
                ...
                status=status,
                priority=random.choices(["low", "medium", "high", "urgent"], weights=[0.15, 0.50, 0.25, 0.10])[0],
    ...
    def create_renewals(self, polizas, usuario):
        ...
        for i, poliza in enumerate(polizas_para_renovar[:num_renewals]):
            ...
            renewal = PolicyRenewal.objects.create(
                ...
                proposed_premium=(
                    Decimal(str(proposed_premium)).quantize(Decimal("0.01")) if status not in ["pending"] else None
                ),
                status=status,
                decision=(
                    random.choice(["renew_same", "renew_different", "pending"])
                    if status in ["in_progress", "quoted"]
                    else "pending"
                ),
                coverage_changes=(
                    "Sin cambios significativos"
                    if random.random() > 0.3
                    else "Se propone ampliar cobertura de responsabilidad civil."
                ),
    ...
    def create_payment_approvals(self, pagos):
        ...
        for pago in pagos_para_aprobar[:num_approvals]:
            ...
            status = random.choices(["pending", "approved", "rejected"], weights=[0.40, 0.45, 0.15])[0]
            ...
            approval = PaymentApproval.objects.create(
                ...
                decision_notes=(
                    "Aprobado según procedimiento estándar."
                    if status == "approved"
                    else ("Rechazado por falta de documentación." if status == "rejected" else "")
                ),
    ...
    def create_calendar_events(self, polizas, facturas, usuario):
        ...
        for poliza in polizas:
            if 0 < poliza.dias_para_vencer <= 60:
                priority = (
                    "critical"
                    if poliza.dias_para_vencer <= 7
                    else ("high" if poliza.dias_para_vencer <= 15 else "medium")
                )
        ...
        for factura in facturas:
            if factura.estado in ["pendiente", "parcial"]:
                ...
                if -30 < dias_para_vencer <= 30:
                    priority = "critical" if dias_para_vencer <= 0 else ("high" if dias_para_vencer <= 7 else "medium")
