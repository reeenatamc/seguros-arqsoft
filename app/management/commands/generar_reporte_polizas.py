...
from app.models import Factura, Poliza
...
class Command(BaseCommand):
    ...
    def crear_hoja_gastos(self, ws, polizas):
        ...
        for poliza in polizas:
            ...
            total_pagado = sum(sum(p.monto for p in f.pagos.filter(estado="aprobado")) for f in facturas)
    ...
    def generar_pdf(self, polizas, filename, estado):
        ...
        for est in estados:
            ...
            resumen_data.append([est.replace("_", " ").title(), str(cantidad), f"${suma_total:,.2f}"])
