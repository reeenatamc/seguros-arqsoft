...
class Command(BaseCommand):
    ...
    def obtener_siniestros_periodo(self, periodo):
        ...
        base_queryset = Siniestro.objects.select_related("poliza", "poliza__compania_aseguradora", "tipo_siniestro")
    ...
    def crear_hoja_resumen(self, ws, siniestros, periodo):
        ...
        resumen_estados = (
            siniestros.values("estado")
            .annotate(cantidad=Count("id"), monto=Coalesce(Sum("monto_estimado"), Decimal("0")))
            .order_by("estado")
        )
    ...
    def crear_hoja_analisis_tipo(self, ws, siniestros):
        ...
        stats_por_tipo = (
            siniestros.values("tipo_siniestro__nombre")
            .annotate(
                cantidad=Count("id"),
                monto_total=Coalesce(Sum("monto_estimado"), Decimal("0")),
    ...
    def crear_hoja_analisis_poliza(self, ws, siniestros):
        ...
        poliza_stats = (
            siniestros.values("poliza__numero_poliza", "poliza__compania_aseguradora__nombre")
            .annotate(cantidad=Count("id"), monto_total=Coalesce(Sum("monto_estimado"), Decimal("0")))
