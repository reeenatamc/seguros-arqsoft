...
from .models import (  # Alias de compatibilidad (deprecados)
    AdjuntoSiniestro,
    BienAsegurado,
    ChecklistSiniestro,
...
class CompaniaAseguradoraForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ["nombre", "ruc", "direccion", "telefono", "email", "contacto_nombre", "contacto_telefono", "activo"]
        widgets = {
...
class CorredorSegurosForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "compania_aseguradora": forms.Select(
                attrs={
                    "class": "form-control",
                }
            ),
...
class GrupoRamoForm(forms.ModelForm):
    ...
    def clean_codigo(self):
        ...
        if tipo_ramo:
            exists = GrupoRamo.objects.filter(tipo_ramo=tipo_ramo, codigo=codigo).exclude(pk=self.instance.pk).exists()
            if exists:
...
class PolizaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "corredor_seguros": forms.Select(
                attrs={
                    "class": "form-select",
                }
            ),
            "grupo_ramo": forms.Select(
                attrs={
                    "class": "form-select",
                }
            ),
            "suma_asegurada": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
            ...
            "porcentaje_deducible": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
            ...
            "deducible_minimo": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
            ...
            "es_gran_contribuyente": forms.CheckboxInput(
                attrs={
                    "class": "form-check-input",
                }
            ),
...
class DetallePolizaRamoForm(forms.ModelForm):
    ...
    def __init__(self, *args, poliza=None, **kwargs):
        ...
        if grupo_ramo:
            ...
            self.fields["subgrupo_ramo"].queryset = SubgrupoRamo.objects.filter(activo=True).order_by(
                "grupo_ramo__nombre", "orden", "nombre"
            )
...
class SiniestroForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "bien_asegurado": forms.Select(
                attrs={
                    "class": "form-select",
                    "data-placeholder": "Seleccione un bien asegurado...",
                }
            ),
            "numero_siniestro": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "Número de siniestro",
                }
            ),
            "subramo": forms.Select(
                attrs={
                    "class": "form-select",
            ...
            "suma_asegurada_bien": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
    ...
    def __init__(self, *args, **kwargs):
        ...
        bienes_qs = BienAsegurado.objects.filter(
            activo=True, poliza__estado__in=["vigente", "por_vencer"]
        ...
        from app.models import DetallePolizaRamo, SubgrupoRamo
        ...
        if poliza:
            ...
            subramos_ids = DetallePolizaRamo.objects.filter(poliza=poliza).values_list("subgrupo_ramo_id", flat=True)
            self.fields["subramo"].queryset = SubgrupoRamo.objects.filter(id__in=subramos_ids).order_by("nombre")
        else:
            # Si no hay póliza, mostrar todos los subramos activos
            self.fields["subramo"].queryset = SubgrupoRamo.objects.filter(activo=True).order_by(
                "grupo_ramo__nombre", "nombre"
            )
        ...
        try:
            if not (instance and instance.email_broker):
                ...
                if "poliza" in self.data:
                    ...
                    if bien_id:
                        bien = (
                            BienAsegurado.objects.select_related("poliza__corredor_seguros").filter(pk=bien_id).first()
                        )
                        if bien:
    ...
    def clean(self):
        ...
        if not bien_asegurado:
            raise ValidationError("Debe seleccionar un Bien Asegurado.")

        # Verificar infraseguro y agregar advertencia (no bloquea)
        self.infraseguro_info = self._verificar_infraseguro(bien_asegurado)

        return cleaned_data

    def _verificar_infraseguro(self, bien):
        """
        Verifica si existe infraseguro en el bien seleccionado.
        Retorna información de la alerta o None si no hay infraseguro.
        """
        if not bien:
            return None

        valor_asegurado = bien.valor_asegurado
        # Valor de reposición: comercial > actual > compra
        valor_reposicion = bien.valor_comercial or bien.valor_actual or bien.valor_compra

        if not valor_asegurado or not valor_reposicion:
            return None

        if valor_asegurado >= valor_reposicion:
            return None

        # Hay infraseguro
        from decimal import Decimal

        porcentaje_cobertura = (valor_asegurado / valor_reposicion * 100).quantize(Decimal("0.01"))
        monto_infraseguro = valor_reposicion - valor_asegurado

        return {
            "tiene_infraseguro": True,
            "valor_asegurado": valor_asegurado,
            "valor_reposicion": valor_reposicion,
            "porcentaje_cobertura": porcentaje_cobertura,
            "monto_infraseguro": monto_infraseguro,
            "mensaje": (
                f"El valor asegurado (${valor_asegurado:,.2f}) es inferior al valor de reposición "
                f"(${valor_reposicion:,.2f}). Cobertura: {porcentaje_cobertura}%. "
                f"Se aplicará regla proporcional en la indemnización."
            ),
        }
...
class SiniestroGestionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "monto_indemnizado": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                }
            ...
            "valor_pagado": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                }
...
class AdjuntoSiniestroForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "requiere_firma": forms.CheckboxInput(
                attrs={
                    "class": "form-check-input",
                }
            ),
...
class BienAseguradoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "codigo_activo": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "Código de activo fijo institucional",
                }
            ),
            "anio_fabricacion": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "min": "1900",
                    "max": "2100",
            ...
            "responsable_custodio": forms.Select(attrs={"class": "form-select"}),
            "valor_compra": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0.01",
            ...
            "valor_asegurado": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0.01",
            ...
            "valor_comercial": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["poliza"].queryset = Poliza.objects.filter(estado__in=["vigente", "por_vencer"]).select_related(
            "compania_aseguradora", "grupo_ramo"
        )
...
class FacturaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "documento_contable": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "contribucion_superintendencia": forms.NumberInput(
                attrs={
                    "class": "form-control bg-slate-50",
                    "step": "0.01",
                    "min": "0",
            ...
            "contribucion_seguro_campesino": forms.NumberInput(
                attrs={
                    "class": "form-control bg-slate-50",
                    "step": "0.01",
                    "min": "0",
...
class ConfiguracionSistemaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            if tipo == "decimal":
                self.fields["valor"].widget = forms.NumberInput(
                    attrs={
                        "class": "form-control",
                        "step": "0.001",
                    }
                ...
                self.fields["valor"].widget = forms.Textarea(
                    attrs={
                        "class": "form-control font-mono text-sm",
                        "rows": 6,
                    }
...
# ==============================================================================
# FORMULARIO DE VISTA PREVIA DE EMAIL
# ==============================================================================


class EmailPreviewForm(forms.Form):
    """
    Formulario para vista previa y edición de emails antes de enviarlos.
    Permite al usuario revisar y modificar el contenido del correo.
    """
    ...
    cc = forms.CharField(
        ...
        help_text="Separar múltiples emails con comas",
    )
    ...
    notas_adicionales = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "class": "form-control",
                "rows": 3,
                "placeholder": "Agregue notas o comentarios adicionales que se incluirán al final del email...",
            }
        ),
        help_text="Estas notas se agregarán al final del mensaje",
    )

    # Campo oculto para el tipo de acción
    accion = forms.CharField(widget=forms.HiddenInput())
    objeto_id = forms.IntegerField(widget=forms.HiddenInput())
